  ...
  _attemptOne(uri, callOpts = {}) {
    ...
    const p = this.srf.createUAC(uri, mergedCallOpts,
      ...
      .catch((err) => {
        ...
        if (err.status == 603) {
          console.log('Here 4 - 603 error', err);
          this.cip.delete(uri);
          this.killCalls();
        } else {
          console.log('Here 5 - other error', err);
        }
...
function oneSuccess(promises) {
  ...
  return Promise.all(promises.map((p) => {
    return p.then((val) => { throw val;}, (err) => err);
  }))
    .then(
      (errors) => {throw errors;},
      (val) => val
