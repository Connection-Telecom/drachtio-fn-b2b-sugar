  ...
  _attemptOne(uri, callOpts = {}) {
    ...
    const p = this.srf.createUAC(uri, mergedCallOpts,
      ...
      .catch((err) => {
        ...
        if (err.status == 603) { // Kill all legs when receiving SIP 603...
          this.cip.delete(uri);
          this.killCalls(); // maybe pass a reason???
          this.res.send(603, 'Decline', {
            headers: {
              'Reason': 'SIP;cause=603;text="Call Declined"'
            }
          });
        }
